...
-(void)setRightbuttonTitleColor:(UIColor *)rightbuttonTitleColor {
    ...
    if (okBtn) {
        [okBtn setTitleColor:_rightbuttonTitleColor forState:UIControlStateNormal];
    }
...
- (void)setTimeBroadcastView
{
    ...
    UILabel *laYear = [[UILabel alloc] initWithFrame:CGRectMake((yearScrollView.bounds.size.width - 12*5)/2 + 12*4-3, kTopViewHeight + 2 * (kTimeBroadcastViewHeight / 5), 20, kTimeBroadcastViewHeight / 5)];
    laYear.text = @"年";
    laYear.font = [UIFont systemFontOfSize:15];
    [timeBroadcastView addSubview:laYear];
    
    
    UILabel *laMonth = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(yearScrollView.frame)+(yearScrollView.bounds.size.width - 12*2)/2 + 12*2 -3, kTopViewHeight + 2 * (kTimeBroadcastViewHeight / 5), 20, kTimeBroadcastViewHeight / 5)];
    laMonth.text = @"月";
    laMonth.font = [UIFont systemFontOfSize:15];
    [timeBroadcastView addSubview:laMonth];
    
    
    UILabel *laDay = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(monthScrollView.frame)+(yearScrollView.bounds.size.width - 12*2)/2 + 12*2-3, kTopViewHeight + 2 * (kTimeBroadcastViewHeight / 5), 20, kTimeBroadcastViewHeight / 5)];
    laDay.text = @"日";
    laDay.font = [UIFont systemFontOfSize:15];
    [timeBroadcastView addSubview:laDay];
    
    
    UILabel *line = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(cancleBtn.frame), kScreenWidth, 0.5)];
    line.backgroundColor = Tex_line;
...
- (UIView *)pageAtIndex:(NSInteger)index andScrollView:(MXSCycleScrollView *)scrollView
{
    ...
    if (scrollView == yearScrollView) {
        ...
        l.frame = CGRectMake((scrollView.bounds.size.width - 12*5)/2, 0, 12*4, scrollView.bounds.size.height/5);
        
        l.text = [NSString stringWithFormat:@"%ld",(long)(_minYear+index)];
    }
    else if (scrollView == monthScrollView)
    {
        l.frame = CGRectMake((scrollView.bounds.size.width - 12*2)/2, 0, 12*2, scrollView.bounds.size.height/5);
