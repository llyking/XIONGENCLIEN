...
//    [self sharedView].defaultMaskType = SVProgressHUDMaskTypeBlack;
}
...
+ (void)showSuccessWithStatus:(NSString*)status {
    ...
    [self performSelector:@selector(dismiss) withObject:status afterDelay:2.5];
}
...
+ (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MAX((float)string.length * 0.06 + 0.3, 0.4);
}
